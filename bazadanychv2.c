...
void ModyfikujRekord(FILE* file, int currentRecord);
...
void Przeglad() {
    ...
    while (fgets(line, sizeof(line), file)) {
        if (currentRecord == 0) {
            printf("\n");
            printf("===== Bieżący rekord: %d =====\n", currentRecord + 1);
        }
        printf("%s", line);  // Wyświetlanie rekordu

        totalRecords++;  // Aktualizacja liczby rekordów
        ...
        printf("Naciśnij M aby modyfikować rekord\n");
        ...
        switch (option) {
            ...
            case 'M':
            case 'm':
                ModyfikujRekord(file, currentRecord);
                break;
            default:
...
void ModyfikujRekord(FILE* file, int currentRecord) {
    fseek(file, 0, SEEK_SET);  // Przejdź na początek pliku

    int recordCount = 0;
    char line[100];

    while (fgets(line, sizeof(line), file)) {
        if (recordCount == currentRecord) {
...
switch (option) {
    ...
    // Przesuń wskaźnik pliku do pozycji, w której zaczyna się rekord do modyfikacji
