...
void Przeglad() {
    ...
    char line[100];
    int akrekord = 0;  // Indeks bie\276\245cego rekordu
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        printf("Naci\230nij M aby modyfikowa\206 rekord\n");
...
void ModyfikujRekord(FILE* file, int akrekord) {
    fseek(file, 0, SEEK_SET);  // Przejdź na pocz\245tek pliku
    ...
    while (fgets(line, sizeof(line), file)) {
        if (liczba == akrekord) {
            break;  // Znaleziono rekord do modyfikacji
        }
        liczba++;
    }
    ...
    Dysk rekord;
    sscanf(line, "%d %s %s %s %lf %lf",
           &rekord.Id, rekord.nazwa, rekord.producent, rekord.typ, &rekord.pojemnosc, &rekord.cena);

    printf("\n===== Modyfikacja rekordu: %d =====\n", akrekord + 1);
    printf("ID: %d\n", rekord.Id);
    printf("Nazwa: %s\n", rekord.nazwa);
    printf("Producent: %s\n", rekord.producent);
    printf("Typ: %s\n", rekord.typ);
    printf("Pojemno\230\206: %.2f\n", rekord.pojemnosc);
    printf("Cena: %.2f\n", rekord.cena);

   printf("\nWybierz pole do edycji:\n");
printf("0. Anuluj edycj\251\n");
printf("1. ID\n");
printf("2. Nazwa\n");
printf("3. Producent\n");
printf("4. Typ\n");
printf("5. Pojemno\230\206\n");
printf("6. Cena\n");
printf("Wybierz opcj\251: ");
...
switch (option) {
    ...
    // Przesu\344 wskaźnik pliku do pozycji, w kt\242rej zaczyna si\251 rekord do modyfikacji
    ...
    printf("\nRekord zosta\210 zaktualizowany.\n");
}
...
void sortowaniebazy() {
    ...
    fseek(file, 0, SEEK_SET);  // Przesuni\251cie na pocz\245tek pliku
    ...
    switch (option3) {
        ...
        case 4:
            ...
            break;
        case 5:
            // Sortuj wed\210ug pojemno\230ci
            for (int i = 0; i < wszrekordy - 1; i++) {
                for (int j = 0; j < wszrekordy - i - 1; j++) {
    ...
    // Wy\230wietlanie posortowanej bazy danych
