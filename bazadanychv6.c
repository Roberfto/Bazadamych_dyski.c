...
void createDatabase() {
  ...
  printf(
      "Utworzono bazę danych o nazwie \"%s\" i dodano przykładowe rekordy.\n",
      nazwa);
  printf("\n");
...
void usuwanieRekordu(struct Rekord *rekord, int liczbaRekordow) {
  ...
  while (fgets(line, sizeof(line), file)) {
    ...
    if (currentId != Id) {
      fprintf(tempFile, "%s",
              line); // Kopiowanie rekordów z wyjątkiem tego o podanym Id
    }
  }
...
void zapiszPlik(struct Rekord *baza, int liczbaRekordow,
